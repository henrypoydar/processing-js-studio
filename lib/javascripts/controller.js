    ...
    with(this) {
          ...
          success: function(json) {
            ...
            for ( var i = 0; i < json.rows.length; i++ ) {
              $('#nav ul.sketches').append(
                "<li><a href='#/sketch/" + json.rows[i].doc._id + "'>" +
                 json.rows[i].doc.name +
                "</a></li>"
              );
              $('#nav ul.manage').append(
                "<li><a href='#/manage/edit/" + json.rows[i].doc._id + "'>" +
                 json.rows[i].doc.name +
                "</a></li>"
              );
            };
      ...
      get('#/sketches', function() { 
        ...
        couchapp.design.view("sketches", {
          ...
          success: function(json) {
            //console.log(json);  
            ctx.partial('views/sketches/index.html', {list: json.rows});
          }
        });
      });
         ...
         couchapp.db.saveDoc(
           {
             ...
             //"_attachments": { ctx.params['sketch_file'] }
           }, {
           success: function(json) {
             flash = { 
