...
void setup() {
  ...
  bg = loadImage("american_flag.gif"); 
...
class Tile {
  ...
  void getColor(float x, float y) {
    ...
    //color c = lerpColor(color(#ffffff), color(#333333), density);  
    density = random(50,100)*0.01;
    
    color c = bg.get(int(x*granularity), int(y*granularity));
    if(red(c) > 250 && blue(c) > 250 && green(c) > 250) {
      color c = lerpColor(color(#ffffff), color(#cccccc), density); 
    }
    noStroke();
    fill(c, density*255);
